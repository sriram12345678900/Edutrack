      ...
      <header className="max-w-6xl mx-auto flex items-center justify-between mb-12 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Showcase your hard-earned achievements.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 px-4">
          {achievements.map((item, index) => (
            <motion.div
              ...
              className={`relative aspect-[3/4] rounded-3xl p-1 group cursor-pointer ${item.unlocked ? 'bg-gradient-to-b from-white/20 to-white/5' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'} shadow-2xl ${item.unlocked ? item.shadow : ''}`}
            >
              {/* Inner card content */}
              <div 
              ...
              >
                ...
                <div 
                  className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 shadow-2xl ${item.unlocked ? 'bg-white/20 ring-4 ring-white/30' : 'bg-slate-50 dark:bg-slate-800 ring-4 ring-slate-700'}`}
                  style={{ transform: 'translateZ(50px)' }}
                >
                  {item.icon}
                </div>
                
                <h3 className={`text-xl font-extrabold mb-2 ${item.unlocked ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`} style={{ transform: 'translateZ(40px)' }}>{item.title}</h3>
