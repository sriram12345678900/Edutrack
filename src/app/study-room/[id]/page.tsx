...
export default function StudyRoom({ params }: { params: { id: string } }) {
  ...
  // Handle incoming shared notes changes
  useEffect(() => {
    if (room) {
      if (isFirstLoad.current) {
        setLocalNotes(room.sharedNotes || "");
        isFirstLoad.current = false;
      } else if (room.notesLastEditedBy !== displayName) {
        setLocalNotes(room.sharedNotes || "");
      }
    }
  }, [room?.sharedNotes, room?.notesLastEditedBy, displayName]);
  ...
  const handleSummarize = async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: [{ role: "user", content: `Please summarize these shared study notes:\n\n${localNotes}` }]
        })
      });
      ...
      <div className="w-full md:w-80 lg:w-96 bg-white dark:bg-[#0c1020] border-l border-slate-200 dark:border-white/10 flex flex-col shadow-xl z-10 shrink-0 h-1/2 md:h-full">
        ...
        <div className="flex border-b border-slate-200 dark:border-white/10 shrink-0">
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === "chat" ? 'border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/10' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5'}`}
          >
            💬 Chat
          </button>
          <button
            onClick={() => setActiveTab("notes")}
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === "notes" ? 'border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/10' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5'}`}
          >
            📝 Shared Notes
          </button>
        </div>
        ...
        {activeTab === "chat" ? (
          <>
            ...
            {/* Chat Input */}
            <div className="p-4 bg-slate-50 dark:bg-black/20 border-t border-slate-200 dark:border-white/10 shrink-0">
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-slate-50 dark:bg-black/20">
            <div className="flex-1 p-4 flex flex-col relative min-h-0">
              <textarea
                ...
                placeholder="Type shared notes here... Everyone in the room will see them."
                className="flex-1 w-full bg-white dark:bg-[#0c1020] border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none shadow-sm"
              />
              <div className="mt-2 flex items-center justify-between shrink-0">
                <span className="text-[10px] font-bold text-slate-400 truncate mr-2">
                  {room.notesLastEditedBy ? `Last edited by: ${room.notesLastEditedBy}` : "No edits yet"}
                </span>
                <button
                  onClick={handleSummarize}
                  disabled={isSummarizing || !localNotes.trim()}
                  className="px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 dark:bg-indigo-900/40 dark:hover:bg-indigo-900/60 dark:text-indigo-300 rounded-lg text-xs font-bold transition-colors flex items-center gap-1 disabled:opacity-50 whitespace-nowrap"
                >
            ...
            {summary && (
              <div className="px-4 pb-4 shrink-0">
                <div className="bg-white dark:bg-white/5 border border-amber-200 dark:border-amber-900/50 rounded-xl p-3 shadow-sm max-h-40 overflow-y-auto">
                  <h4 className="text-xs font-black text-amber-600 dark:text-amber-500 mb-1 flex items-center gap-1">
                    ✨ AI Summary
                  </h4>
